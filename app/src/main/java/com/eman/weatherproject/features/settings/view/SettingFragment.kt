...
class SettingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        settingsViewModel = ViewModelProvider(this, settingsViewModelFactory)[SettingViewModel::class.java]

        settings = settingsViewModel.getStoredSettings()!!
        setupUI()

        binding.notiEnable.setOnClickListener { settings.notification = true
            settingsViewModel.setSettingsSharedPrefs(settings)
        }

        binding.notiDisable.setOnClickListener { settings.notification = false
            settingsViewModel.setSettingsSharedPrefs(settings)


            val dialogBuilder = AlertDialog.Builder(requireContext())
            dialogBuilder.setMessage(getString(R.string.warning))
                .setCancelable(false).setPositiveButton(getString(R.string.ok)) { dialog, id -> dialog.cancel() }
